...
if(isset($_POST['getDetail'])) {
    try {
        ...
        $queryJ = "SELECT id_materi, file_materi, materi, kegiatan, keterangan FROM tbl_materi WHERE id_mapel = '$id_escaped' AND `" . $dateCol . "` = '$tglskr_escaped'";
        ...
        if (mysqli_num_rows($resJ) > 0) {
            ...
            $existingAbsen = [];
            $queryAbsen = "SELECT no_induk, status FROM tbl_absen WHERE id_mapel = '$id_escaped' AND tanggal = '$tglskr_escaped'";
            ...
            echo '<div id="ringkasanJurnal" class="card mb-3"><div class="card-header bg-light d-flex justify-content-between align-items-center">Ringkasan Jurnal Hari Ini <button type="button" id="btnEditJurnal" class="btn btn-sm btn-primary">Edit Jurnal</button></div><div class="card-body">';
            echo '<div class="mb-3">';
            if (!empty($file)) {
            ...
            echo '<div class="mt-2"><a class="btn btn-sm btn-outline-danger" href="delete-materi.php?id='.$idMateri.($file?('&file='.rawurlencode($file)):'').'" onclick="return confirm(\'Yakin mau menghapus isian jurnal ini?\');">Hapus Isian Jurnal</a></div>';
...
<div>
    ...
    <h5 class="mt-2 border-bottom pb-2">Form Jurnal Baru</h5>
    ...
    <form id="formJurnalInput" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="materi" id="materi" class="form-control" rows="2" required placeholder="Pokok bahasan..."></textarea>
        </div>
        
        <div class="mb-3">
            <label for="kegiatan" class="form-label fw-bold">Kegiatan Pembelajaran</label>
            <textarea name="kegiatan" id="kegiatan" class="form-control" rows="2" required placeholder="Contoh: Diskusi, Praktikum..."></textarea>
        </div>
                ...
                if($siswaQuery && mysqli_num_rows($siswaQuery) > 0) {
                    ...
                    while($s = mysqli_fetch_assoc($siswaQuery)) {
                        ...
                        echo "<tr>
                                ...
                                <td class='text-center'><input class='absen-radio' type='radio' data-letter='h' name='absen[".$nis."]' value='Hadir'></td>
                                <td class='text-center'><input class='absen-radio' type='radio' data-letter='i' name='absen[".$nis."]' value='Ijin'></td>
                                <td class='text-center'><input class='absen-radio' type='radio' data-letter='s' name='absen[".$nis."]' value='Sakit'></td>
                                <td class='text-center'><input class='absen-radio' type='radio' data-letter='d' name='absen[".$nis."]' value='Dispen'></td>
                                <td class='text-center'><input class='absen-radio' type='radio' data-letter='a' name='absen[".$nis."]' value='Alpha'></td>
                                <td class='text-center'><input class='absen-radio' type='radio' data-letter='t' name='absen[".$nis."]' value='Telat'></td>
                              </tr>";
                    }
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="keterangan" id="keterangan" placeholder="Opsional..."></textarea>
        </div>
...
(function() {
    ...
    if(form) {
        form.onsubmit = function(e) {
            ...
            fetch('simpanmateri.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.text())
            .then(data => {
                msg.innerHTML = data; // Tampilkan pesan dari PHP

                // Cek jika sukses
                if(data.includes('alert-success') || data.includes('Berhasil')) {
                    form.reset();
                    btn.disabled = true;
