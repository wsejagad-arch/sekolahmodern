...
if (isset($_POST['submit'])) {
    //definisikan variabel dulu
	  ...
	  if($error != UPLOAD_ERR_NO_FILE) {
		...
		mysqli_query($conn, "UPDATE tbl_setting SET nama_sekolah='$nmlembaga', alamat='$alamat', nama_pimpinan='$pimpinan', nip_pimpinan='$nip', logo='$cekfoto', gemini_api_key='$gemini_api_key' WHERE id='$id'");
	...
	<?php } else if($error === UPLOAD_ERR_NO_FILE) {
		mysqli_query($conn, "UPDATE tbl_setting SET nama_sekolah='$nmlembaga', alamat='$alamat', nama_pimpinan='$pimpinan', nip_pimpinan='$nip', gemini_api_key='$gemini_api_key' WHERE id='$id'");
		mysqli_query($conn, "INSERT INTO tbl_log(waktu, isi_log) VALUES('$tglskr', '$isilog')"); ?>
		<script>
			  Swal.fire({
			  position: 'top-end',
