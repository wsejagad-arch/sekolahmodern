...
if (isset($_POST['submit'])) {
    //definisikan variabel dulu
	  ...
	  if($error != UPLOAD_ERR_NO_FILE) {
		...
		mysqli_query($conn, "UPDATE tbl_guru g LEFT JOIN tbl_mapel_ampu m ON g.no_induk=m.no_induk SET g.no_induk='$nip', g.nama_guru='$nami', g.status_kepegawaian='$status_kepegawaian', g.foto='$cekfoto', g.status='$status', m.no_induk='$nip' WHERE g.id_guru='$idguru'");
	...
	<?php } else if($error === UPLOAD_ERR_NO_FILE) {
		mysqli_query($conn, "UPDATE tbl_guru g LEFT JOIN tbl_mapel_ampu m ON g.no_induk=m.no_induk SET g.no_induk='$nip', g.nama_guru='$nami', g.status_kepegawaian='$status_kepegawaian', g.status='$status', m.no_induk='$nip' WHERE g.id_guru='$idguru'");
		mysqli_query($conn, "INSERT INTO tbl_log(waktu, isi_log) VALUES('$tglskr', '$isilog')"); ?>
		<script>
			  Swal.fire({
			  position: 'top-end',
	...
	<?php } else { ?>
		<script>Swal.fire('Gagal', 'merubah data guru!', 'error')</script>
	<?php }
}
?>
