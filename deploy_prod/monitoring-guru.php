...
$isBKGuru = array_keys($guruBKSiswa);
...
foreach ($allRows as $r) {
    ...
    $kurang = $isBK ? ($guruBKSiswa[$nip] ?? 0) < 150 : ($guruWeeklyMenit[$nip] ?? 0) < 540;
    if ($kurang) {
        if (!$r['id_materi']) $guruBelumIsiKurang[$nip] = $r['nama_guru'];
        else $guruSudahIsiKurang[$nip] = $r['nama_guru'];
    }
}
                    ...
                    <tbody>
                        ...
                        foreach ($allRows as $row) {
                            ...
                            if ($isBK) {
                                ...
                                $badgeKuota = $terpenuhi
                                    ? "<span class='badge badge-success badge-pill' style='font-size:11px;'><i class='fas fa-check-circle'></i> Siswa Terpenuhi ({$totalSiswa} siswa)</span>"
                                    : "<span class='badge badge-pill badge-blink-orange' style='font-size:11px;'><i class='fas fa-users'></i> Kurang Siswa ({$totalSiswa}/150)</span>";
                            } else {
                                $terpenuhi = ($guruWeeklyMenit[$nip] ?? 0) >= 540;
                                $infoText  = "<i class='fas fa-stopwatch'></i> Hari ini: {$jpHari} JP &nbsp;|&nbsp; Per minggu: {$jpMinggu} JP";
                                $badgeKuota = $terpenuhi
                                    ? "<span class='badge badge-success badge-pill' style='font-size:11px;'><i class='fas fa-check-circle'></i> Jam Terpenuhi ({$jpMinggu} JP/minggu)</span>"
                                    : "<span class='badge badge-pill badge-blink-orange' style='font-size:11px;'><i class='fas fa-clock'></i> Jam Kurang ({$jpMinggu} JP/minggu)</span>";
                            }

                            if ($row['id_materi']) {
                                $sudahIsi++;
                                $badgeJurnal = "<span class='badge badge-primary badge-pill' style='font-size:11px;'><i class='fas fa-check-circle'></i> Sudah Isi Jurnal</span>";
                                $rowClass = $terpenuhi ? '' : 'table-warning';
                            } else {
                                $belumIsi++;
                                $badgeJurnal = "<span class='badge badge-danger badge-pill badge-blink' style='font-size:11px;'><i class='fas fa-exclamation-triangle'></i> Belum Isi Jurnal</span>";
