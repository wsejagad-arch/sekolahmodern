...
$result = $conn->query("SELECT * FROM posts WHERE created_at <= NOW() ORDER BY created_at DESC LIMIT 5");
...
<body>
    ...
    <div class="container">
        <main class="main-content">
            <div id="berita" class="latest-posts" style="margin-bottom: 4rem;">
                <h2 class="section-title" style="font-size: 2rem; margin-bottom: 1.5rem; text-align: left;">Postingan Terbaru</h2>
                <?php if ($result && $result->num_rows > 0): ?>
                        ...
                        <article class="post-card" style="margin-bottom: 2rem;">
                            ...
                            if(!empty($row['image'])) {
                                if(filter_var($row['image'], FILTER_VALIDATE_URL)) {
                                    $image_src = $row['image'];
                                } elseif(file_exists("uploads/posts/" . $row['image'])) {
                                    $image_src = "uploads/posts/" . $row['image'];
                                } elseif(file_exists("uploads/" . $row['image'])) {
                                    $image_src = "uploads/" . $row['image'];
                                }
                            }
                            
                            // Jika tidak ada image di kolom image, coba cari dari content (regex img tag)
                            if(empty($image_src)) {
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <div class="post-card" style="padding: 3rem; text-align: center; margin-bottom: 2rem;">
                        <h2 class="post-title">Belum ada informasi.</h2>
                        <p class="post-excerpt">Informasi terbaru akan segera diperbarui oleh admin.</p>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <aside class="sidebar">
            <!-- Widget Pengumuman -->
            <div class="post-card" style="padding: 1.5rem; margin-bottom: 2rem; border-top: 4px solid var(--primary);">
                <h3 style="margin-bottom: 1.5rem; font-size: 1.25rem; font-weight: 800; display: flex; align-items: center; gap: 10px;">
                    <span style="font-size: 1.5rem;">📢</span> Pengumuman Terbaru
                </h3>
                <?php
                $annRes = $conn->query("SELECT * FROM announcements ORDER BY created_at DESC LIMIT 3");
                if($annRes && $annRes->num_rows > 0):
                    while($ann = $annRes->fetch_assoc()):
                        $badgeClass = 'badge-' . $ann['type'];
                ?>
                    <div style="margin-bottom: 1.25rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                            <span style="font-size: 0.75rem; color: var(--text-muted); font-weight: 600;"><?= date('d M Y', strtotime($ann['created_at'])) ?></span>
                            <span class="badge <?= $badgeClass ?>" style="padding: 2px 8px; border-radius: 4px; font-size: 0.7rem; font-weight: bold; 
                                <?php 
                                    if($ann['type'] == 'Penting') echo 'background: #fee2e2; color: #ef4444;';
                                    elseif($ann['type'] == 'Kegiatan') echo 'background: #dcfce3; color: #22c55e;';
                                    else echo 'background: #f1f5f9; color: #64748b;';
                                ?>
                            "><?= $ann['type'] ?></span>
                        </div>
                        <h4 style="font-size: 1rem; margin-bottom: 0.5rem; color: var(--text-main); line-height: 1.4;"><?= htmlspecialchars($ann['title']) ?></h4>
                        <p style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.5;"><?= nl2br(htmlspecialchars($ann['content'])) ?></p>
                    </div>
                <?php 
                    endwhile;
                else:
                ?>
                    <p style="font-size: 0.9rem; color: var(--text-muted); text-align: center;">Belum ada pengumuman.</p>
                <?php endif; ?>
            </div>
            ...
            <div class="ad-space" style="padding: 0; background: #f8fafc; border: 1px solid #e2e8f0; overflow: hidden; display: flex; align-items: center; justify-content: center; min-height: 600px;">
                <?php if(!empty($setting['ads_code'])): ?>
                    <?= $setting['ads_code'] ?>
                <?php else: ?>
                    <div style="width: 100%; height: 600px; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 2rem;">
                        <div style="font-size: 4rem; margin-bottom: 1rem;">🏫</div>
                        <h3 style="font-size: 1.5rem; margin-bottom: 1rem; color: white;"><?= htmlspecialchars($setting['ppdb_title']) ?></h3>
                        <p style="font-size: 0.9rem; opacity: 0.9; margin-bottom: 2rem;"><?= nl2br(htmlspecialchars($setting['ppdb_subtitle'])) ?></p>
                        <a href="https://wa.me/<?= preg_replace('/\D/', '', $setting['phone']) ?>" class="btn" style="background: white; color: #2563eb;"><?= htmlspecialchars($setting['ppdb_btn_text']) ?></a>
    ...
    <!-- Section Pengaduan -->
    <section style="background: #f1f5f9; padding: 5rem 2rem; border-top: 1px solid var(--border);">
        <div class="nav-container" style="display: grid; grid-template-columns: 1fr 1.2fr; gap: 4rem; align-items: center;">
            <div>
                <div style="display: inline-block; padding: 0.5rem 1rem; background: rgba(37,99,235,0.1); color: var(--primary); border-radius: 20px; font-size: 0.85rem; font-weight: 700; margin-bottom: 1rem; text-transform: uppercase; letter-spacing: 1px;">Layanan Suara Masyarakat</div>
                <h2 style="font-size: 2.5rem; margin-bottom: 1.5rem; font-weight: 800; line-height: 1.2;">Layanan Pengaduan Online</h2>
                <p style="color: var(--text-muted); font-size: 1.1rem; line-height: 1.7; margin-bottom: 2rem;">
                    Kami berkomitmen untuk terus meningkatkan kualitas pelayanan. Jika Anda memiliki keluhan, saran, atau masukan, silakan sampaikan melalui formulir ini. Pesan Anda akan langsung terkirim ke WhatsApp resmi sekolah.
                </p>
                <div style="display: flex; gap: 1.5rem;">
                    <div style="flex: 1; background: white; padding: 1.5rem; border-radius: 12px; border: 1px solid var(--border);">
                        <div style="font-size: 1.5rem; margin-bottom: 0.5rem;">📱</div>
                        <div style="font-weight: 800; font-size: 1rem;">WhatsApp Hotlink</div>
                        <div style="color: var(--text-muted); font-size: 0.9rem;"><?= htmlspecialchars($setting['phone']) ?></div>
                    </div>
                    <div style="flex: 1; background: white; padding: 1.5rem; border-radius: 12px; border: 1px solid var(--border);">
                        <div style="font-size: 1.5rem; margin-bottom: 0.5rem;">⏱️</div>
                        <div style="font-weight: 800; font-size: 1rem;">Waktu Respon</div>
                        <div style="color: var(--text-muted); font-size: 0.9rem;">Senin - Jumat (07:00 - 15:00)</div>
                    </div>
                </div>
            </div>
            <div style="background: white; padding: 3rem; border-radius: 20px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.05); border: 1px solid var(--border);">
                <form id="formPengaduan">
                    <div class="form-group" style="margin-bottom: 1.5rem;">
                        <label style="font-weight: 700; margin-bottom: 0.5rem; display: block;">Nama Lengkap / Wali Murid</label>
                        <input type="text" id="pengaduan_nama" class="form-control" placeholder="Masukkan nama Anda" required style="background: #f8fafc;">
                    </div>
                    <div class="form-group" style="margin-bottom: 1.5rem;">
                        <label style="font-weight: 700; margin-bottom: 0.5rem; display: block;">Status / Hubungan</label>
                        <select id="pengaduan_status" class="form-control" style="background: #f8fafc;">
                    ...
                    <div class="form-group" style="margin-bottom: 2rem;">
                        <label style="font-weight: 700; margin-bottom: 0.5rem; display: block;">Isi Pengaduan / Pesan</label>
                        <textarea id="pengaduan_pesan" class="form-control" rows="4" placeholder="Tuliskan pengaduan atau masukan Anda di sini..." required style="background: #f8fafc;"></textarea>
                    </div>
                    <button type="submit" class="btn" style="width: 100%; padding: 1rem; border-radius: 12px; font-size: 1.1rem; display: flex; align-items: center; justify-content: center; gap: 10px;">
    ...
    <script>
        document.getElementById('formPengaduan').addEventListener('submit', function(e) {
            ...
            const nama = document.getElementById('pengaduan_nama').value;
            const status = document.getElementById('pengaduan_status').value;
            const pesan = document.getElementById('pengaduan_pesan').value;
            const phone = "<?= preg_replace('/\D/', '', $setting['phone']) ?>";
            
            const text = `Halo Admin Sekolah,\n\nSaya ingin menyampaikan pengaduan/masukan:\n\n*Nama:* ${nama}\n*Status:* ${status}\n*Pesan:* ${pesan}\n\nTerima kasih.`;
