...
$result = $conn->query("SELECT g.*, m.nama_mapel 
                         FROM tbl_guru g 
...
while($row = $result->fetch_assoc()) {
    ...
    if($row['jabatan'] == 'Kepala Sekolah') {
        $group_name = 'Pimpinan Sekolah';
    } elseif($row['jabatan'] == 'Tenaga Kependidikan') {
        $group_name = 'Tenaga Kependidikan (TU)';
    } else {
        $group_name = $row['nama_mapel'] ?: 'Umum';
    }
    $grouped_teachers[$group_name][] = $row;
}
?>
<!DOCTYPE html>
<html lang="id">
...
<body>
    ...
    <div class="hero" style="background-image: url('uploads/<?= htmlspecialchars($setting['hero_bg']) ?>'); padding: 6rem 2rem; min-height: auto; margin-bottom: 4rem; position: relative;">
        <div class="hero-overlay" style="background: linear-gradient(135deg, rgba(15,23,42,0.85) 0%, rgba(37,99,235,0.6) 100%);"></div>
        <div class="hero-content nav-container" style="justify-content: center; text-align: center; padding: 0; position: relative; z-index: 2;">
            <div class="hero-text" style="max-width: 800px; padding-bottom: 0; margin: 0 auto;">
                <div style="display: inline-block; padding: 0.5rem 1.5rem; background: rgba(255,255,255,0.2); backdrop-filter: blur(5px); border-radius: 30px; margin-bottom: 1.5rem; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; font-size: 0.85rem; border: 1px solid rgba(255,255,255,0.3);">Tim Pengajar</div>
                <h1 style="font-size: 3.5rem; margin-bottom: 1rem; line-height: 1.2;">Profil Tenaga Pendidik</h1>
                <p style="font-size: 1.25rem; opacity: 0.9; margin-bottom: 0;">Mengenal lebih dekat para pendidik luar biasa yang berdedikasi membimbing dan menginspirasi masa depan generasi bangsa.</p>
            </div>
        </div>
    </div>

    <div class="container" style="display: block; padding-bottom: 4rem;">
        <?php if (!empty($grouped_teachers)): ?>
            <?php foreach($grouped_teachers as $mapel => $teachers): ?>
                <div class="mapel-group" style="margin-bottom: 4rem;">
                    <h2 class="section-title" style="font-size: 2rem; margin-bottom: 2rem; padding-bottom: 1rem; border-bottom: 3px solid var(--primary); display: inline-block;"><?= htmlspecialchars($mapel) ?></h2>
                            ...
                            <div class="teacher-card">
                                ...
                                <div class="teacher-info">
                                    ...
                                    <span class="teacher-subject">
                                        <?php 
                                        if($row['jabatan'] != 'Guru') {
                                            echo htmlspecialchars($row['jabatan']);
                                        } else {
                                            echo htmlspecialchars($row['nama_mapel'] ?: 'Umum');
                                        }
                                        ?>
                                    </span>
                                    <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: 5px;">NIP: <?= htmlspecialchars($row['nip_guru'] ?: '-') ?></p>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <div style="text-align: center; width: 100%; padding: 3rem; background: var(--card-bg); border-radius: 12px; border: 1px solid var(--border);">
                <h2>Belum ada data guru.</h2>
                <p class="post-excerpt">Data profil guru akan segera ditambahkan oleh admin.</p>
            </div>
        <?php endif; ?>
    </div>
