...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['nama_guru'])) {
    ...
    if(empty($message)) {
        if($id_guru > 0) {
            $stmt = $conn->prepare("UPDATE tbl_guru SET no_induk=?, nama_guru=?, id_mapel=?, no_wa=?, status_kepegawaian=?, walas=?, nip_guru=?, foto=?, status=?, is_guru_bk=? WHERE id_guru=?");
            $stmt->bind_param("ssissssssii", $no_induk, $nama_guru, $id_mapel, $no_wa, $status_kepegawaian, $walas, $nip_guru, $foto_name, $status, $is_guru_bk, $id_guru);
        } else {
            $stmt = $conn->prepare("INSERT INTO tbl_guru (no_induk, nama_guru, id_mapel, no_wa, status_kepegawaian, walas, nip_guru, foto, status, is_guru_bk) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssissssssi", $no_induk, $nama_guru, $id_mapel, $no_wa, $status_kepegawaian, $walas, $nip_guru, $foto_name, $status, $is_guru_bk);
        }
                                    ...
                                    <?php while($row = $teachers->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div style="font-size: 0.75rem; color: #64748b;"><?= htmlspecialchars($row['status_kepegawaian']) ?> <?= $row['is_guru_bk'] ? ' | Guru BK' : '' ?></div>
                                            </td>
                                            <td>
