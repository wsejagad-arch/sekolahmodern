...
$sqlguru = mysqli_query($conn, "SELECT * FROM tbl_guru WHERE id_sekolah=$idSekolah AND no_induk='$nipguru'");
          ...
          <div class="list-item">
            ...
            <div class="main">
              <ul class="list p-3">
                ...
                <li class="menu-das">
                  <a href="home.php?page=jurnalguru">
                    <img src="../../img/logo dash.png" alt="" class="icon2" />
                    <span class="text-das align-text-top">Lihat Jurnal</span>
                  </a>
                </li>
                <li class="menu-das">
  <a href="cetak_jurnal.php" target="_blank">
    <img src="../../img/logo dash.png" alt="" class="icon2" />
    <span class="text-das align-text-top">Cetak Jurnal</span>
  </a>
</li>
              </ul>
            </div>
      ...
      <div class="main-content">
        ...
        <div class="container-fluid d-flex justify-content-between mt-2">
		  ...
		  <?php
			if($dataguru['foto'] == "") { ?>
			<img src="../../img/no-photo.png" alt="" class="d-inline-block img-profile"/>
		<?php
			} else { ?>
			<img src="../../foto/<?= $dataguru['foto']; ?>" alt="" class="d-inline-block img-profile"/>
		<?php
			}
		  ?>
            <p class="text-profile align-text-top d-none d-sm-none d-md-block">Hai, <?= $_SESSION["nama_guru"]; ?></p>
          </a>
        </div>
        <hr class="mt-0 mx-5" />
        ...
        <section id="Jadwal">
          <div class="container">
			  ...
			  <?php
				if (isset($_GET["sukses"])) { 
                    echo '<div class="alert alert-success" id="error-alert">
						Berhasil mengirim jurnal pembelajaran
                    </div>';
                } else if (isset($_GET["gagal"])) { 
                    echo '<div class="alert alert-danger" id="error-alert">
                    <strong>Gagal!</strong> mengirim jurnal.
                    </div>';
                } else if (isset($_GET["hapusmateri"])) { 
                    echo '<div class="alert alert-success" id="error-alert">
                    <strong>Berhasil!</strong> menghapus jurnal.
                    </div>';
                } else if (isset($_GET["gagalhapusmateri"])) { 
                    echo '<div class="alert alert-danger" id="error-alert">
                    <strong>Gagal!</strong> menghapus jurnal.
                    </div>';
                }
			  ?>
			...
			<?php
				// tampilkan data jadwal pada tabel mapel ampu
				$idSekolah = mt_current_school_id();
				$sql = mysqli_query($conn, "SELECT * FROM tbl_mapel_ampu m JOIN tbl_guru g ON m.no_induk = g.no_induk AND g.id_sekolah = m.id_sekolah WHERE m.id_sekolah=$idSekolah AND m.no_induk='$nipguru' AND m.hari='$hariini' ORDER BY m.jam_mulai ASC");
                ...
                <div class="card-jadwal"> <!-- ini card jadwal -->
                  <div class="card-content mx-auto">
					...
					?>
                      <p class="pt-1">Kelas : <?= $data['kelas']; ?></p>
                    </div>
                      ...
                      <p class="p"><?= $data['jam_mulai']; ?> WIB s.d <?= $data['jam_selesai']; ?> WIB</p>
						...
						<?php
							// Tampilkan File Materi
                              $idSekolah = mt_current_school_id();
                              $mat = mysqli_query($conn, "SELECT * FROM tbl_materi WHERE id_sekolah=$idSekolah AND id_mapel='$idmapel' AND `tanggal`='$tglskr'");
							$jumdata = mysqli_num_rows($mat);
							if($jumdata < 1) { ?>
								<p class="mapel">Belum ada file materi!</p>
						<?php
							} else {
							while ($dmat = mysqli_fetch_array($mat)) { ?>
								<div class="d-flex card-profile" href="#">
								<img src="../../img/pdf.png" alt="" class="d-inline-block img-profile card-image"/>
								<p class="mapel"><a href="../../materi/<?= $dmat['file_materi']; ?>" target="_blank"><?= $dmat['file_materi']; ?></a></p>
								<div class="button-unduh">
								  <a class="download text-decoration-none text-danger" href="delete-materi.php?id=<?= $dmat['id_materi']; ?>&file=<?= $dmat['file_materi']; ?>" onclick="return confirm('Yakin mau menghapus file jurnal ini?');">Hapus</a>
								</div>
								<div class="clear"></div>
								</div>
							<?php
                  ...
                  <!-- Button trigger modal -->
					<button type="button" class="btn btn-lapor w-100" data-bs-toggle="modal" data-bs-target="#show" data-id="<?= $data['id_mapel']; ?>">Isi Jurnal</button>
			...
			<?php 	
				...
				<div class="col-md-6 col-lg-5 col-xl-12 m-0">
				<div class="card-jadwal">
				<div class="card-content mx-auto">
				<div class="alert alert-danger mt-4">
					Hai <strong><?= $_SESSION['nama_guru']; ?></strong>, belum ada jadwal untuk hari ini. 
				</div>
				</div>
				</div>
				</div>
		...
		<div class="modal-dialog modal-lg">
		  <div class="modal-content p-3">
			<div class="modal-header">
			  <span class="modal-title" id="">Jurnal</span>
