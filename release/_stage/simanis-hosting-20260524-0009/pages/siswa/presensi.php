    ...
    <div class="w-full max-w-2xl mx-auto p-4 sm:p-6">
        <div class="bg-white border rounded-xl p-3 shadow-sm">
            ...
            <header class="flex justify-between items-center mb-2">
                <div>
                    <h1 class="text-base font-bold text-gray-700">
                        <i class="fas fa-fingerprint text-blue-400 mr-2"></i>Presensi Saya
                    </h1>
                    <p class="text-xs text-gray-400 mt-0.5">
                        <?= htmlspecialchars($namaSiswa) ?> &middot; Kelas <?= htmlspecialchars($kelas) ?>
                    </p>
                </div>
                <a href="siswa.php" class="text-blue-500 hover:text-blue-700 text-sm font-medium">
            ...
            <div class="mb-2 bg-white border border-gray-200 rounded-lg p-2">
                <h2 class="text-xs font-bold text-blue-700 uppercase tracking-wide mb-2">
                    <i class="fas fa-clock mr-1"></i>Presensi Mandiri — <?= htmlspecialchars($hariIndo) ?>, <?= tgl_indo($tglHariIni) ?>
                </h2>

                <?php if (empty($jadwalHariIni)): ?>
                    <p class="text-sm text-gray-500 italic"><i class="fas fa-calendar-times mr-1 text-gray-400"></i>Tidak ada jadwal pelajaran hari ini.</p>
                <?php else: ?>
                    <?php
                    // Tentukan mapel terakhir hari ini (jam_selesai terbesar = jam pulang)
                    $lastMapelId = null;
                    $_maxSel = '';
                    ...
                    ?>
                    <div class="space-y-1" id="jadwalList">
                        ...
                        ?>
                            <div class="flex items-center justify-between border-b border-gray-100 last:border-0 bg-white px-2 py-2">
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-gray-700 truncate"><?= htmlspecialchars($jdw['nama_mapel']) ?></p>
                                    <p class="text-[11px] text-gray-400">
                                        <i class="fas fa-clock mr-0.5"></i><?= $jam ?>
                                        <?php if (!empty($jdw['nama_guru'])): ?>
                                            &nbsp;&middot;&nbsp;<i class="fas fa-chalkboard-teacher mr-0.5"></i><?= htmlspecialchars($jdw['nama_guru']) ?>
                                        <?php endif; ?>
                                    </p>
                                </div>
                                <div class="ml-2 shrink-0 flex flex-col items-end gap-1">
                                    <?php if ($sudahAbsen): ?>
                                        <span class="text-[10px] font-semibold px-2 py-0.5 rounded <?= $byGuru ? 'badge-guru' : ($bySiswa ? 'badge-siswa' : 'bg-green-100 text-green-700') ?>">
                                            <?= htmlspecialchars($jdw['status_absen']) ?>
                                            <?php if ($bySiswa): ?> <i class="fas fa-user-check ml-0.5" title="Absen mandiri siswa"></i><?php endif; ?>
                                            <?php if ($byGuru):  ?> <i class="fas fa-chalkboard-teacher ml-0.5" title="Dikonfirmasi guru"></i><?php endif; ?>
                                        </span>
                                    <?php else:
                                        $isLast = ((int)$jdw['id_mapel'] === (int)$lastMapelId);
                                    ?>
                                        <button type="button"
                                            class="btn-absen-mandiri text-[11px] font-semibold px-2 py-0.5 rounded bg-blue-500 text-white hover:bg-blue-600 active:scale-95 transition-all shadow-none"
                                            ...
                                            <i class="fas <?= $isLast ? 'fa-sign-out-alt' : 'fa-fingerprint' ?> mr-1"></i><?= $isLast ? 'Absen Pulang' : 'Absen' ?>
                                        </button>
                                        <div id="time-ind-<?= (int)$jdw['id_mapel'] ?>" class="text-[10px] mt-0.5"></div>
                                </div>
                            <?php endif; ?>
                            </div>
                    </div>
        ...
        <?php if ($isKetuaKelas): ?>
            <div class="mb-2 bg-white border border-gray-200 rounded-lg p-2">
                <h2 class="text-sm font-bold text-amber-700 uppercase tracking-wide mb-1 flex items-center gap-2">
                    <i class="fas fa-user-shield"></i>Konfirmasi Kehadiran Guru — Ketua Kelas
                </h2>
                <p class="text-xs text-amber-600 mb-3">Konfirmasikan kehadiran guru mata pelajaran pada hari ini.</p>

                <?php if (empty($jadwalHariIni)): ?>
                    <p class="text-sm text-gray-500 italic"><i class="fas fa-calendar-times mr-1"></i>Tidak ada jadwal hari ini.</p>
                <?php else: ?>
                    <div class="space-y-1.5" id="konfirmasiList">
                        <?php foreach ($jadwalHariIni as $jdw):
                        ...
                        ?>
                            <div class="bg-white rounded-lg p-2 border-b border-gray-100 last:border-0" id="konfirm-card-<?= $idMapelK ?>">
                                <div class="flex items-start justify-between gap-2 mb-1">
                                    <div>
                                        <p class="text-sm font-semibold text-gray-800"><?= htmlspecialchars($jdw['nama_mapel']) ?></p>
                                ...
                                <?php if ($konf): ?>
                                    <div class="text-xs text-gray-400 mb-1.5"><i class="fas fa-check-circle text-green-400 mr-1"></i>Sudah dikonfirmasi
                                        <?php if (!empty($konf['catatan'])): ?> &middot; <em><?= htmlspecialchars($konf['catatan']) ?></em><?php endif; ?>
                                    </div>
                                    <button type="button" onclick="hapusKonfirmasi(<?= $idMapelK ?>)"
                                        class="text-xs text-red-400 hover:text-red-600 underline">
                                        <i class="fas fa-undo mr-0.5"></i>Ubah / Batalkan
                                    </button>
                                <?php else: ?>
                                    <div class="flex flex-wrap gap-0.5" id="konfirm-buttons-<?= $idMapelK ?>">
                                        <?php foreach (['Hadir', 'Telat', 'Izin', 'Tidak Hadir Tanpa Tugas', 'Tidak Hadir Ada Tugas'] as $optK): ?>
                                            <button type="button" onclick="kirimKonfirmasi(<?= $idMapelK ?>, this)"
                                                ...
                                                class="text-[10px] font-medium px-2 py-0.5 rounded border <?= konfirmasiButtonColor($optK) ?> transition-all active:scale-95">
        ...
        <div id="cameraModal" role="dialog" aria-modal="true" aria-labelledby="cameraTitle">
            <div class="bg-white rounded-2xl shadow-2xl w-min(380px,95vw) p-5 mx-4 max-w-sm w-full">
                ...
                <p class="text-xs text-gray-500 text-center mb-3" id="cameraSubtitle">Posisikan wajah Anda di dalam oval</p>
                ...
                <!-- Status deteksi -->
                <div id="faceStatus" class="mt-3 text-center text-sm font-medium text-gray-500">
                    <i class="fas fa-spinner fa-spin mr-1"></i>Memuat model deteksi…
                </div>

                <!-- Petunjuk -->
                <ul class="mt-2 text-xs text-gray-400 list-none space-y-0.5 text-center">
                    <li><i class="fas fa-check-circle text-green-400 mr-1"></i>Hadapkan wajah ke kamera (depan)</li>
                    <li><i class="fas fa-check-circle text-green-400 mr-1"></i>Pastikan cahaya cukup</li>
                    <li><i class="fas fa-times-circle text-red-400 mr-1"></i>Wajah miring/samping tidak terdeteksi</li>
                </ul>
                ...
                <div class="mt-4 flex gap-2">
                    <button id="btnAbsenKonfirm"
                        disabled
                        class="flex-1 bg-blue-500 text-white text-sm font-semibold py-2 rounded-xl opacity-50 cursor-not-allowed transition-all">
                        <i class="fas fa-check mr-1"></i>Konfirmasi Absen
                    </button>
                    <button onclick="closeCamera()"
                        class="px-4 py-2 border rounded-xl text-sm text-gray-600 hover:bg-gray-50">
                        Batal
                    </button>
                </div>
                <div id="modalMsg" class="mt-2 text-xs text-center"></div>
            </div>
        </div>

        <!-- Pilih Bulan (rekap historis) -->
        <form method="GET" action="presensi.php" class="mb-2">
            <label class="block text-xs font-semibold text-gray-500 mb-1">
                <i class="fas fa-calendar-alt mr-1 text-blue-400"></i>Pilih Bulan
            </label>
        ...
        <?php else: ?>
            ...
            <div class="mb-2">
                <h2 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                    Rekap — <?= bulanIndo($bulan) ?>
                </h2>
                <table class="w-full text-xs border-collapse border border-gray-200 bg-white">
                    <thead>
                        <tr class="bg-gray-50">
                            <th class="border border-gray-200 px-3 py-2 text-left font-semibold text-gray-600">Status</th>
                            <th class="border border-gray-200 px-3 py-2 text-right font-semibold text-gray-600">Jumlah</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="border border-gray-200 px-3 py-2 text-green-700 font-medium">Hadir</td>
                            <td class="border border-gray-200 px-3 py-2 text-right font-bold text-green-700"><?= $summary['Hadir'] ?></td>
                        </tr>
                        <tr>
                            <td class="border border-gray-200 px-3 py-2 text-yellow-700 font-medium">Ijin</td>
                            <td class="border border-gray-200 px-3 py-2 text-right font-bold text-yellow-700"><?= $summary['Ijin'] ?></td>
                        </tr>
                        <tr>
                            <td class="border border-gray-200 px-3 py-2 text-red-700 font-medium">Alpha</td>
                            <td class="border border-gray-200 px-3 py-2 text-right font-bold text-red-700"><?= $summary['Alpha'] ?></td>
                        </tr>
                        <tr>
                            <td class="border border-gray-200 px-3 py-2 text-blue-700 font-medium">Sakit/Dispen</td>
                            <td class="border border-gray-200 px-3 py-2 text-right font-bold text-blue-700"><?= $summary['Sakit'] + $summary['Dispen'] ?></td>
                        </tr>
                    </tbody>
                </table>
                <?php if ($totalPertemuan > 0): ?>
                    <div class="mb-2 bg-gray-50 rounded-lg p-2">
                        <div class="flex justify-between text-[11px] text-gray-500 mb-1">
                            <span><i class="fas fa-chart-line mr-1 text-blue-400"></i>Kehadiran</span>
                            <span class="font-bold <?= ($pctHadir >= 75) ? 'text-green-600' : 'text-red-500' ?>">
                                <?= $pctHadir ?>%
                            </span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-1.5">
                            <div class="progress-bar <?= ($pctHadir >= 75) ? 'bg-green-500' : 'bg-red-400' ?> h-1.5 rounded-full"
                ...
                <?php if (empty($detailList)): ?>
                    <div class="text-center py-10">
                        ...
                        <p class="text-gray-500 text-sm">Tidak ada data presensi pada <?= bulanIndo($bulan) ?>.</p>
                    </div>
                <?php else: ?>

                    <div class="mb-1 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">
                        <i class="fas fa-list-ul mr-1 text-blue-400"></i>Detail Pertemuan
                    </div>
                    ...
                    <div class="divide-y divide-gray-100 max-h-72 overflow-y-auto pr-1">
                        ...
                        ?>
                                <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest pt-2 pb-1">
                                    <?= htmlspecialchars($hariTgl) ?>
                                </div>
                            <?php endif; ?>

                            <div class="flex items-center justify-between bg-white px-2 py-2">
                                <div class="flex-1 min-w-0">
                                ...
                                <span class="ml-3 text-[10px] font-semibold px-2 py-0.5 rounded <?= $badge ?>">
