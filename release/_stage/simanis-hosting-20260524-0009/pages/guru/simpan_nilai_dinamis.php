...
foreach ($nilai as $idItem => $bySiswa) {
  $idItem = (int)$idItem;
  $validItem = mysqli_query($conn, "SELECT id FROM tbl_penilaian_item WHERE {$tenantPenilaian} AND id={$idItem} AND id_mapel={$idmapel} LIMIT 1");
  if (!$validItem || mysqli_num_rows($validItem) === 0) {
    continue;
  }
  foreach ($bySiswa as $nis => $val) {
    $nis = mysqli_real_escape_string($conn, $nis);
    $validSiswa = mysqli_query($conn, "SELECT no_induk FROM tbl_siswa WHERE {$tenantSiswa} AND no_induk='{$nis}' LIMIT 1");
    if (!$validSiswa || mysqli_num_rows($validSiswa) === 0) {
      continue;
    }
