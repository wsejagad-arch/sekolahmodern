        ...
        <div class="card-body">
          ...
          <div class="row">
            <?php foreach ($templates as $t):
              ...
              $appBase = function_exists('get_base_path') ? rtrim(get_base_path(), '/') : '';
              if ($appBase === '' && !empty($_SERVER['SCRIPT_NAME'])) {
                  $sn = str_replace('\\', '/', $_SERVER['SCRIPT_NAME']);
                  if (preg_match('#^(.*)/pages/guru/[^/]+$#', $sn, $m)) {
                      $appBase = ($m[1] === '/' || $m[1] === '') ? '' : $m[1];
                  }
              }
              $shareUrl = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http')
                        . '://' . $_SERVER['HTTP_HOST'] . $appBase . '/pages/siswa/twibbon.php?id=' . $t['id'];
            ?>
